...
from .submodule import conv3d_bn_relu, conv3d_bn, deconv3d_bn
...
class Hourglass(nn.Module):
    ...
    def forward(self, x, presqu=None, postsqu=None):
        ...
        if presqu is not None:
            out1 = self.conv5(out)
            presqu = F.interpolate(presqu, size=out1.size()[2:], mode='nearest')
            post = F.relu(out1 + presqu, inplace=True)
        else:
            out1 = self.conv5(out)
            pre = F.interpolate(pre, size=out1.size()[2:], mode='nearest')
            post = F.relu(out1 + pre, inplace=True)
...
class PSMAggregator(nn.Module):
    ...
    def forward(self, raw_cost):
        ...
        out1 = out1 + F.interpolate(cost0, size=out1.size()[2:], mode='nearest')

        out2, pre2, post2 = self.dres3(out1, pre1, post1)
        out2 = out2 + F.interpolate(cost0, size=out2.size()[2:], mode='nearest')

        out3, pre3, post3 = self.dres4(out2, pre2, post2)
        out3 = out3 + F.interpolate(cost0, size=out3.size()[2:], mode='nearest')
