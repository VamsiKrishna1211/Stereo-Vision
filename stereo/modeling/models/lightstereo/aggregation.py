...
class Aggregation(nn.Module):
    ...
    def forward(self, x, features_left):
        ...
        out1 = self.conv5(conv4)
        out2 = self.redir2(conv2)
        conv5 = F.interpolate(out1, size=out2.size()[2:], mode='nearest') + out2
        out3 = self.conv6(conv5)
        out4 = self.redir1(x)

        conv6 = F.interpolate(out3, size=out4.size()[2:], mode='nearest') + out4
